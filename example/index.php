...
<body>
    ...
    <script>
        ...
        loginForm.addEventListener('submit', async (e) => {
            ...
            const response = await fetch('http://api.local/api/login', {
                method: 'POST',
                body: JSON.stringify({
        ...
        logoutButton.addEventListener('click', async (e) => {
            ...
            const response = await fetch('http://api.local/api/logout', {
                method: 'POST',
                body: JSON.stringify({
        ...
        getTasksButton.addEventListener('click', async (e) => {
            ...
            const response = await fetch("http://api.local/api/tasks", {
                headers: {
        ...
        list.addEventListener('click', async (e) => {
            ...
            if (id) {
                ...
                const response = await fetch(`http://api.local/api/tasks/${id}`, {
                    headers: {
                        "Authorization": "Bearer " + localStorage.getItem("access_token")
                    }
                });
                ...
                if (response.status == 200) {
                    ...
                    const response = await fetch('http://api.local/api/refresh', {
                        method: 'POST',
                        body: JSON.stringify({
                            token: localStorage.getItem("refresh_token")
                        })
